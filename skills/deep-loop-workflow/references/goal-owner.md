...
Use the supplied action and remaining host budget first. `host_budget.remaining_call_time_ms` is this invocation limit; it is shorter than the whole-run horizon. Yield before it expires so the host can measure and resume the same thread. A setup-only or partial inline-stage yield is allowed when necessary; retain the existing attempt and exact next action. Token accounting includes cached input and updates when a measured process returns. After completing an action, obtain `next-action --json`; retain its action and verify its identity against the host frame. A changed owner/generation yields to the host; reading identity does not transfer ownership. Refresh after external waits, a stale token/fence response, or changed context. Never retry a failed mutation blindly. If a genuinely required binding is missing, obtain one fresh state snapshot and retain the needed information. The kernel remains authoritative for lease, scope, budget, breaker, prerequisites, review and completion proof.
