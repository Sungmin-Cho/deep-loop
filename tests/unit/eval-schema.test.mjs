...
test('validateResult requires the five verdicts and seven metadata fields', () => {
  const result = {
    ...
    meta: { model: 'none:fixture', harness: 'none:fixture', profile: 'deep-loop-current-v1.24', kernel_version: '1.24.0', node: 'v26', runtime: 'fixture', task_bank_sha256: '0'.repeat(64) },
  };
...
test('profile runtime contract is exact and default-deny', () => {
  const profile = {
    id: 'deep-loop-current-v1.24', driver: 'fixture', model: 'none:fixture',
...
test('published schemas execute against the same positive and negative contract vectors', async () => {
  ...
  const profile = JSON.parse(readFileSync(join(process.cwd(), 'evals', 'profiles', 'deep-loop-current-v1.24.json'), 'utf8'));
