...
export function validateProfile(profile) {
  ...
  const exactProfiles = {
    'deep-loop-current-v1.24': ['fixture','none:fixture','none:fixture',false],
...
export function validateResult(result, bank = undefined) {
  ...
  const comparisonProfiles = new Set(['host-native','deep-loop-kernel-minimal','deep-loop-current-v1.24','deep-loop-experimental']);
  ...
  for (const row of result.profile_comparison_stub.filter(item => item.agency_loss_incident)) {
    ...
    const counterpart = result.profile_comparison_stub.some(item => item.task_id === row.task_id
      && ['host-native','deep-loop-current-v1.24'].includes(item.profile) && item.outcome_pass === true);
